...
import { Search, MoreHorizontal, CheckCircle, XCircle, Trash2, ChevronDown, Check, Plus } from 'lucide-react';
import {
  Card,
  CardHeader,
  CardContent,
...
export default function AdminProgramsPage() {
  ...
  const categories = categoriesResponse?.data?.categories?.map((c: any) => c.name) || [];
  ...
  const filteredPrograms = programs.filter((program: any) => {
    const categoryMatch = activeCategory === 'All' || program.category?.name === activeCategory;
    const searchMatch = (
      program.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      program.coach?.user?.fullName?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    return categoryMatch && searchMatch;
  });

  const handleProgramAction = async (programId: string, action: 'approve' | 'reject' | 'activate' | 'deactivate') => {
    try {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                className={cn('rounded-full', activeCategory === 'All' && 'bg-muted font-bold')}
              ...
              <DropdownMenu onOpenChange={(open) => !open && setIsAddingCategory(false)}>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className={cn("rounded-full flex items-center gap-1", activeCategory !== 'All' && 'bg-muted font-bold')}>
                    ...
                    <>
                      {categories.map((cat: string) => (
                        <DropdownMenuItem key={cat} onSelect={(e) => e.preventDefault()}>
                          <span onClick={() => handleCategorySelect(cat)} className="flex-grow p-2 -m-2">{cat}</span>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6"
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onSelect={(e) => { e.preventDefault(); setIsAddingCategory(true); }}>
                  ...
                  <TableRow key={program.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          ...
                          <AvatarFallback>{program.coach?.user?.fullName?.charAt(0)}</AvatarFallback>
                        </Avatar>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                          <DropdownMenuItem onClick={() => handleViewDetails(program)}>View Details</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          {program.status === 'PENDING_APPROVAL' && (
                            <>
                              <DropdownMenuItem onClick={() => handleProgramAction(program.id, 'approve')}>
                                <CheckCircle className="mr-2 h-4 w-4" /> Approve
                              </DropdownMenuItem>
                              <DropdownMenuItem className="text-destructive" onClick={() => handleProgramAction(program.id, 'reject')}>
                                <XCircle className="mr-2 h-4 w-4" /> Reject
                              </DropdownMenuItem>
                            </>
                          )}
                          {program.status === 'ACTIVE' && (
                            <DropdownMenuItem onClick={() => handleProgramAction(program.id, 'deactivate')}>
                              <XCircle className="mr-2 h-4 w-4" /> Deactivate
                            </DropdownMenuItem>
                          )}
                          {program.status === 'INACTIVE' && (
                            <DropdownMenuItem onClick={() => handleProgramAction(program.id, 'activate')}>
      ...
      <AlertDialog open={!!categoryToDelete} onOpenChange={() => setCategoryToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the &quot;{categoryToDelete}&quot; category. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
