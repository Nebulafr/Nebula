...
const CreateSessionInputSchema = z.object({
  ...
  studentIds: z.array(z.string()).describe('An array of student IDs attending the session.'),
  scheduledTime: z.string().describe('The scheduled start time of the session in ISO 8601 format.'),
  title: z.string().describe('The title of the session.'),
  description: z.string().describe('A brief description of the session.'),
  attendeeEmails: z.array(z.string()).describe('An array of attendee email addresses for the calendar invite.'),
});
...
const createCalendarEventTool = ai.defineTool(
  {
    ...
    description: 'Creates a Google Calendar event and generates a Google Meet link for it.',
