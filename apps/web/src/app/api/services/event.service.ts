...
import { prisma, Event, User, Prisma } from "@nebula/database";
import {
...
export class EventService {
  ...
  findBySlug = async (slug: string): Promise<any> => {
    const event = await prisma.event.findUnique({
      where: {
        slug,
      },
  ...
  update = async (userId: string, userRole: string, id: string, data: UpdateEventData): Promise<any> => {
    if (userRole !== "ADMIN") {
      throw new UnauthorizedException("Admin access required");
    }
  ...
  remove = async (userId: string, userRole: string, id: string): Promise<any> => {
  ...
  registerForEvent = async (userId: string, eventId: string): Promise<any> => {
  ...
  unregisterFromEvent = async (userId: string, eventId: string): Promise<any> => {
