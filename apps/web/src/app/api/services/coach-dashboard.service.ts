import { prisma, Prisma, TransactionType, TransactionSourceType, TransactionStatus } from "@nebula/database";
...
export class CoachDashboardService {
  ...
  async getSessions(
  ...
  ): Promise<{ sessions: CoachDashboardSession[] }> {
    // Get coach profile
    const coach = await prisma.coach.findUnique({
      where: { userId },
      select: { id: true },
    ...
    const transformedSessions: CoachDashboardSession[] = sessions.map(
      (session) => ({
        id: session.id,
        title: session.title,
  ...
  async getPrograms(userId: string, status: string = "all"): Promise<{ programs: DashboardProgram[] }> {
    // Get coach profile
    const coach = await prisma.coach.findUnique({
      where: { userId },
      select: { id: true },
    ...
    const transformedPrograms: DashboardProgram[] = programs.map(
      (program) => ({
        id: program.id,
        title: program.title,
        slug: program.slug,
  ...
  ): Promise<{ students: FormattedStudent[]; pagination: any }> {
  ...
  async getEarnings(userId: string): Promise<Array<{ month: string; earnings: number }>> {
    const coach = await prisma.coach.findUnique({
      where: { userId },
  ...
  async getRecentPayouts(userId: string, limit: number = 5): Promise<{ payouts: Payout[]; payoutsThisMonth: number }> {
    const coach = await prisma.coach.findUnique({
      where: { userId },
      select: { id: true },
    });
