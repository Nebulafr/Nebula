...
export class PaymentService {
    async handleCheckoutSessionCompleted(session: Stripe.Checkout.Session): Promise<any> {
    ...
    async handleProgramEnrollment(programId: string, userId: string, stripeSessionId: string | null, cohortId?: string): Promise<any> {
    ...
    async handleEventRegistration(eventId: string, userId: string, stripeSessionId: string | null): Promise<any> {
        const student = await prisma.student.findUnique({ where: { userId } });
        if (!student) throw new NotFoundException(`Student not found for userId: ${userId}`);
    ...
    async processRefund(type: "PROGRAM" | "SESSION" | "EVENT", id: string): Promise<any> {
